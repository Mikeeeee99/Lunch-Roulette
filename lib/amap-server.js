...
async function fetchAmapPage({ key, longitude, latitude, radius, cityCode, page, fetchImpl }) {
  ...
  if (isValidAmapCityCode(cityCode)) {
    searchParams.set("city", cityCode);
    searchParams.set("citylimit", "true");
  }
...
function normalizeResolvedLocation(coordinate, regeocode = {}) {
  ...
  const country = textValue(component.country);
  const province = textValue(component.province);
  const city = textValue(component.city);
  const cityCode = textValue(component.citycode);
  ...
  return {
    ...
    supported: isMainlandChinaAdcode(adcode)
      && isMainlandChinaCoordinate(coordinate.longitude, coordinate.latitude),
  };
}
...
export async function handleNearbyRestaurantsRequest(request, bindings = {}, fetchImpl = fetch) {
  ...
  const adcode = textValue(body?.adcode);
  const cityCode = textValue(body?.cityCode);
  if (body?.coordinateSystem !== "gcj02"
    || !Number.isFinite(longitude)
    || !Number.isFinite(latitude)
    || !ALLOWED_RADII.has(radius)
    || !isMainlandChinaAdcode(adcode)
    || (cityCode && !isValidAmapCityCode(cityCode))) {
    return publicError("INVALID_REQUEST", "位置或搜索范围不正确。", 400);
  }
  if (!isMainlandChinaCoordinate(longitude, latitude)) {
    return publicError("UNSUPPORTED_REGION", "目前仅支持中国大陆地区。", 403);
  }
  ...
  try {
    const firstPage = await fetchAmapPage({ key, longitude, latitude, radius, cityCode, page: 1, fetchImpl });
    ...
    if (total > AMAP_PAGE_SIZE && AMAP_MAX_PAGES > 1) {
      const secondPage = await fetchAmapPage({ key, longitude, latitude, radius, cityCode, page: 2, fetchImpl });
      rawPois.push(...(Array.isArray(secondPage.pois) ? secondPage.pois : []));
    }

    const restaurants = normalizeAmapPois(rawPois, cityCode);
    return jsonResponse({
      restaurants,
      meta: {
