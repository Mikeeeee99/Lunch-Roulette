...
export function isShanghaiCoordinate(longitude, latitude) {
  const lng = Number(longitude);
  const lat = Number(latitude);
  return Number.isFinite(lng)
    && Number.isFinite(lat)
    && lng >= SHANGHAI_BOUNDS.minLongitude
    && lng <= SHANGHAI_BOUNDS.maxLongitude
    && lat >= SHANGHAI_BOUNDS.minLatitude
    && lat <= SHANGHAI_BOUNDS.maxLatitude;
}

export function isShanghaiAdcode(adcode) {
  return typeof adcode === "string" && adcode.startsWith("31");
}
...
export function normalizeAmapPoi(poi) {
  ...
  if (adcode && !isShanghaiAdcode(adcode)) return null;
  if (!isShanghaiCoordinate(longitude, latitude)) return null;
...
export function normalizeAmapPois(pois) {
  if (!Array.isArray(pois)) return [];
  const unique = new Map();
  for (const poi of pois) {
    const normalized = normalizeAmapPoi(poi);
